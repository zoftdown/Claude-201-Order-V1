...
**เพิ่มล่าสุด (V3.6 · 2026-08-03): เชื่อมระบบ Brief ลึกขึ้น** (คู่กับฝั่ง Brief commit 30c8b2d — API export/detail + badge/ปุ่ม)
- **"FK จริง" ข้ามระบบ = `brief_job_id` (IntegerField) เดิมจาก V2.9** — Brief เป็นคนละ Django project คนละ DB (เชื่อมผ่าน internal API เท่านั้น) ทำ DB-level FK ไม่ได้; `design_doc_number` (text) เป็น source of truth ที่แสดงบนใบงาน **ไม่ถูกแตะเลย** → rollback การจับคู่ = เคลียร์ brief_job_id กลับ NULL จบ
- **autocomplete เลขใบงานออกแบบ (อัปเกรด):** dropdown โชว์เพจของใบออกแบบเพิ่ม (บรรทัดล่าง) + เลือกแล้ว **auto-fill**: ชื่อลูกค้า/ลิงก์แชท (เฉพาะช่องที่ยังว่าง — ไม่ทับที่พิมพ์เอง) + แหล่งที่มา (เซ็ตเสมอเมื่อ map ได้ — `source_label` ฝั่ง Brief ตรงกับค่า `SOURCE_CHOICES` ฝั่งนี้ 10 เพจ) — ฟังก์ชัน `autofillFromBriefJob` ใน order_form.html. *Brief ยังไม่มีช่องชื่องาน/ad_source — มีเมื่อไหร่ค่อยเติมที่ฟังก์ชันนี้*
- **`/create/?design_job=<id>`** (ปุ่ม "🧾 สร้างออเดอร์จากใบนี้" ฝั่ง Brief): `_fetch_brief_job` ดึงใบงานจาก `GET /api/jobs/<id>/` → prefill เลขใบงาน+ลูกค้า+ลิงก์แชท+เพจ + hidden `brief_job_id` ผูกให้เลย (best-effort: Brief ล่ม/id มั่ว = ฟอร์มเปล่าปกติ) — hidden ยังคงค่าตอน POST ไม่ผ่าน validation (`request.POST.brief_job_id` มาก่อนใน template)
- **helper `_brief_api_get(path)`** ใน views.py — GET API ฝั่ง Brief พร้อม token, คืน None เมื่อปิดฟีเจอร์/ล่ม (ใช้ร่วม: proxy autocomplete, prefill, conversion, command)
- **command `link_brief_jobs`** — จับคู่ `design_doc_number` (text ใบเก่า) → `brief_job_id`: เลขตรงเป๊ะ (ตัด case/ช่องว่าง; เลขล้วน "123" = "D-123") **และ** ชื่อลูกค้าตรง/ใกล้เคียง (normalize เท่ากัน หรือ substring ยาว ≥3) — กำกวม/ชื่อไม่ตรง = ข้ามพร้อมโชว์รายการ. **default dry-run** (โชว์ทุกคู่ให้คนตรวจ) เขียนจริงใส่ `--confirm` (+ ยิง order_ref กลับ Brief เฉพาะใบงานที่ฝั่งโน้นยังว่าง ครั้งเดียวต่อใบงาน) — รันบน VPS: `venv/bin/python manage.py link_brief_jobs` (ต้อง source .env ก่อนเหมือน migrate)
